 ...
 * @li read a PNG file into a de-interlaced unsigned char or double array
 * @li write an unsigned char or double array to a PNG file
 *
 ...
 * @todo process the data as double before quantization
...
/**
 * @brief (de)interlace a double array
 *
 ...
 * @return new array
 */
static double *_io_png_inter(const double *data, size_t csize, size_t nc,
                            _io_png_inter_t option)
{
    ...
    if (1 == nc || 1 == csize) {
        /* duplicate */
        tmp = _IO_PNG_SAFE_MALLOC(csize * nc, double);
        memcpy(tmp, data, csize * nc * sizeof(double));
        return tmp;
    }

    size = nc * csize;
    tmp = _IO_PNG_SAFE_MALLOC(size, double);
...
#define _IO_PNG_ANY2FLT(MAX) do {                       \
        ...
        flt_data = _IO_PNG_SAFE_MALLOC(size, double);    \
        max = (double) (MAX);                            \
        for (i = 0; i < size; i++)                      \
            flt_data[i] = (double) (data[i]) / max;      \
        return flt_data;                                \
    } while (0)

/**
 * @brief convert png_byte array to double
 *
 ...
 */
static double *_io_png_byte2flt(const png_byte * data, size_t size)
{
...
/**
 * @brief convert unsigned char array to double
 *
 * See _io_png_byte2flt()
 */
static double *_io_png_uchar2flt(const unsigned char *data, size_t size)
{
    _IO_PNG_ANY2FLT(UCHAR_MAX);
}

/**
 * @brief convert unsigned short array to double
 *
 * See _io_png_byte2flt()
 */
static double *_io_png_ushrt2flt(const unsigned short *data, size_t size)
{
...
/**
 * @brief convert double array to png_byte
 *
 ...
 */
static png_byte *_io_png_flt2byte(const double *flt_data, size_t size)
{
    /* png_byte is 8bit data unsigned, [0..255] */
...
/**
 * @brief convert double array to unsigned char
 *
 * See _io_png_flt2byte()
 */
static unsigned char *_io_png_flt2uchar(const double *flt_data, size_t size)
{
    _IO_PNG_FLT2ANY(unsigned char, UCHAR_MAX);
}

/**
 * @brief convert double array to unsigned short
 *
 * See _io_png_flt2byte()
 */
static unsigned short *_io_png_flt2ushrt(const double *flt_data, size_t size)
{
    _IO_PNG_FLT2ANY(unsigned short, USHRT_MAX);
}

/**
 * @brief convert double gray to rgb
 *
 * @param data array to convert
 * @param size array size
 * @return converted array (via realloc())
 */
static double *_io_png_gray2rgb(double *data, size_t size)
{
    assert(NULL != data && 0 != size);

    data = _IO_PNG_SAFE_REALLOC(data, 3 * size, double);
    memcpy(data + size, data, size * sizeof(double));
    memcpy(data + 2 * size, data, size * sizeof(double));

    return data;
}

/**
 * @brief convert double rgb to gray
 *
 * Y = Cr* R + Cg * G + Cb * B
 * with
 ...
 */
static double *_io_png_rgb2gray(double *data, size_t size)
{
    size_t i;
    double *r, *g, *b;
    ...
    data = _IO_PNG_SAFE_REALLOC(data, size, double);
 ...
 * @return pointer to an array of double pixels, abort() on error
 *
 * @todo don't loose 16bit info
 * @todo use enums?
 */
static double *_io_png_read(const char *fname,
                           ...
                           io_png_opt_t opt)
{
    ...
    case IO_PNG_OPT_RGB:
        if (4 == nc || 2 == nc) {
            /* strip alpha channel ... */
            data = _IO_PNG_SAFE_REALLOC(data, nx * ny * (nc - 1), double);
            nc = (nc - 1);
        }
        if (1 == nc) {
            /* gray->rgb */
...
/**
 * @brief read a PNG file into a double array with some options
 *
 ...
 */
double *io_png_read_flt_opt(const char *fname,
                           size_t * nxp, size_t * nyp, size_t * ncp,
                           io_png_opt_t opt)
{
    double *flt_data;
...
/**
 * @brief read a PNG file into a double array
 *
 * The image is read into an array with the deinterlaced channels,
 * with values in [0,1].
 *
 ...
 * @return pointer to an array of pixels, abort() on error
 */
double *io_png_read_flt(const char *fname,
                       size_t * nxp, size_t * nyp, size_t * ncp)
{
 ...
 * @param data non interlaced (RRRGGGBBBAAA) double image array
 ...
 * @todo handle 16bit
 */
static void _io_png_write(const char *fname, const double *data,
...
/**
 * @brief write a double array into a PNG file with some options
 *
 ...
 * @return void, abort() on error
 */
void io_png_write_flt_opt(const char *fname, const double *data,
                          size_t nx, size_t ny, size_t nc, io_png_opt_t opt)
{
 ...
 * @param nx, ny, nc number of columns, lines and channels of the image
 */
void io_png_write_flt(const char *fname, const double *data,
 ...
 * converted to double in the [0,1] interval before being saved as 8bit
